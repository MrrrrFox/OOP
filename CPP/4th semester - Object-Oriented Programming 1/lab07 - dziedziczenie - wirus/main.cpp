/*
Zadanie służy przetestowaniu konstruktorów kopiujących i przenoszących
oraz operatora = w dziedziczeniu.

Należy zaimplementować następujące klasy:

* RNA (rna.h) - implementuje dowolną sekwencję nukleotydów - łańcuch RNA.
  Nukleotydy są nowym typem enum. Deklaracja tej klasy oraz typu 'Nukleotyp' jest dana, 
  należy jedynie dopisać plik "rna.cpp". 
  Uwaga! Można mutować łańcuch.

* DNA (dna.h) - klasa pochodna klasy RNA implementująca łańcuch DNA.
  Łańcuch DNA jest ciągiem dwóch łańcuchów nukleotydów, połączonych słabymi wiązaniami.
  Klasa ma być minimalistyczna - wystarczy się w niej ograniczyć do konstruktora i funkcji wyświetlającej.
...
* Virus (virus.h) - klasa implemetuje wirus o następujących cechach:
  ...
  - "wiek" wirusa - ilość wyprodukowanych pokoleń (bez mutacji)
    ...
    Na przykład, jeśli wiek wirusa vir1 wynosi 1, to nowy wirus vir2 utworzony przez
    instrukcję 'Virus vir2(vir1)' bedzie miał wiek 0, zaś vir1 wiek 2.

* AnimalVirus (animalvirus.h) - klasa pochodna klasy Virus, 
  implementująca wirusy przenoszone przez zwierzęta. 
  Dodatkową cechą klasy AnimalVirus jest:
  - nazwa zwierzęcia, które jest hostem wirusa.

UWAGI:
*  Aby program działał poprawnie, odpowiednie klasy muszą posiadać 
   niedomyślne konstruktory kopiujące/przenoszące oraz operatory=.
*  Proszę pamiętać, o oprogramowaniu antyplagiatowym używanym na 
   platformie VPL - proszę pracować samodzielnie.
*  Szczegóły implementacji klas należy odczytać z funkcji main().
*/ 
//---------------------------------------------------------------------
...
int main() {
	...
	cout << "RNA1: " << rna1 << "RNA2: " << rna2 << endl;
	// mutujemy RNA - przesuwamy rotacyjnie w lewo
	cout << "MUTE RNA2: " << rna2.mute() << endl;
...
MUTE RNA2: A T C C A G A G T A C T C A C 
